...
class ProtoMetaLearner(MetaLearner):
    ...
    - optimizer
    """
    def __init__(self, meta_epoch, valid_check_epoch, patience, valid_tasks, batch_size, first_eval=1, logger=logger.get_logger('base'), time_begin=time.time()) -> None:
        super().__init__()
        self.logger = logger
        self.timer = timer()
        self.timer.initialize(time_begin, 60 * 100)
    ...
    def meta_fit(self, train_pipe, valid_pipe, cls_num):
        ...
        for epoch in range(self.meta_epoch):
            if epoch == self.valid_check_epoch + 1 or (epoch > self.valid_check_epoch and epoch % 20 == 0):
...
class ProtoMultiManager():
    ...
    def load(self, model_dir, device='auto'):
        if(os.path.isdir(model_dir) != True):
            raise ValueError(('The model directory provided is invalid. Please'
                + ' check that its path is valid.'))
        self.load_model(model_dir, device=device)
